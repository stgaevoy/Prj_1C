...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	//dz
	// Формирование движений регистра накопления dz_ОбъемЗакупокПоНакопительнойКарте
	Движения.dz_ОбъемЗакупокПоНакопительнойКарте.Записывать = Истина;
	Движение 					= Движения.dz_ОбъемЗакупокПоНакопительнойКарте.Добавить();
	Движение.Период 			= Дата;
	Движение.НакопительнаяКарта = dz_КартаЛояльности;

	Если Валюта.Пустая() Тогда
		Движение.СуммаЗакупок = Товары.Итог("Сумма");
	Иначе
		...
		Если Курс = 0 Тогда
			Движение.СуммаЗакупок = Товары.Итог("Сумма");
		Иначе
			Движение.СуммаЗакупок = Товары.Итог("Сумма") / Курс;
		КонецЕсли;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	dz_СуммаСкидкиИтоги = Товары.Итог("dz_СуммаСкидкиАвтоматической") + Товары.Итог("dz_СуммаСкидкиРучной");
	Если dz_СуммаСкидки <> dz_СуммаСкидкиИтоги Тогда
		dz_СуммаСкидки = dz_СуммаСкидкиИтоги;
	КонецЕсли;
